"""Mostly adapted from Dinov2 repo"""
...
def get_vit_lr_decay_rate(name, lr_decay_rate=1.0, num_layers=12, force_is_backbone=False, chunked_blocks=False):
    ...
    if name.startswith("backbone") or force_is_backbone:
        ...
        if any(s in name for s in strings):
            layer_id = 0
...
def get_maybe_fused_params_for_submodel(cfg: dict[str, Any], model: nn.Module):
    params_groups = get_params_groups_with_decay(
        model=model,
        lr_decay_rate=cfg["optimizer"]["layerwise_decay"],
        patch_embed_lr_mult=cfg["optimizer"]["patch_embed_lr_mult"],
    )
...
def get_params_groups(cfg: dict[str, Any], model: nn.Module | nn.ModuleDict):
    all_params_groups = []
    model_iterator = model.values() if isinstance(model, nn.ModuleDict) else model.children()
    for m in model_iterator:
        all_params_groups += get_maybe_fused_params_for_submodel(cfg, m)
